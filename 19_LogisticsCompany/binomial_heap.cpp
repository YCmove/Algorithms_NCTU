...
void binomialHeapInsert(int x, int pos) 
{ 
	// Create a new node and do union of 
	// this node with root 
	root = unionBHeaps(root, createNode(x, pos)); 
} 
...
{ 
	...
	{ 
		if ((curr->sibling)->T.val < min) 
		{ 
			min = (curr->sibling)->T.val; 
			min_node_prev = curr; 
...
{ 
	...
	// Recur for child 
	Node *res = findNode(h->child, pos); 
	if (res != NULL) 
	return res; 

	return findNode(h->sibling, pos); 
} 

// Function to decrease the value of old_val 
// to new_val 
void decreaseKeyBHeap(Node *H, int pos, 
							int new_val) 
{ 
	// First check element present or not 
	Node *node = findNode(H, pos); 
	...
	while (parent != NULL && node->T.val < parent->T.val) 
	{ 
		swap(node->T, parent->T); 
		node = parent; 
...
Node *binomialHeapDelete(Node *h, int pos) 
{ 
	...
	decreaseKeyBHeap(h, pos, INT_MIN); 
...
// 	binomialHeapInsert(10,1); 
// 	binomialHeapInsert(20,2); 
// 	binomialHeapInsert(30,3); 
// 	binomialHeapInsert(40,4); 
// 	binomialHeapInsert(50,5); 

// 	cout << "The heap is:\n"; 
// 	display(root); 

// 	// Delete a particular element from heap 
// 	root = binomialHeapDelete(root, 3); 
