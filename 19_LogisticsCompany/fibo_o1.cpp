...
void binomialdijkstra(int src) //dijkstra's using binomial
{
    ...
    while(!(heap->isEmpty()))
    {
        ...
        while(travel!=NULL)
        {
            ...
            if(dist[x]==INT8_MAX) //=>insert it into heap for first time
            {
                dist[x] = dist[u] + travel->weight;
...
void fibonaccidijkstra(int src) //dijkstra's using fibonacci
{
    ...
    int i;
    for(i=0;i<v;i++)
        dist[i] = INT8_MAX; //initially, every vertex is unreachable
    ...
    while(!(heap->isEmpty()))
    {
        ...
        while(travel!=NULL)
        {
            ...
            if(dist[x]==INT8_MAX) //=> insert this vertex into the heap for the first time
            {
                dist[x] = dist[u] + travel->weight;
